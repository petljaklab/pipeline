...
There are 93 genome-wide SNP sites that allow discrimination between all COSMIC/CCLE cell lines. The variants and their genotypes in the ~1k cell lines can be found at:

```
/gpfs/data/petljaklab/resources/hg19/pipeline_resources/somatic_celline/qc/\
1k_cells_genotyping/reference_genotypes.txt
```

We check for variants matching these reference sites using Bcftools (ver. 1.20). Similarity is calculated using a simple scoring system of 0.5 points per haplotype of difference. For example, if a variant is genotyped as A/A in the sample but is G/G in the reference, both haplotypes are different and the two samples have a distance of 1. For a given sample, the most likely cell line is determined by the reference line with the smallest total distance to the sample. Normally we observe a very clear closest match with a mutational distance under 10 and all other lines matching with distances multiple times higher. A line with no matches in the cell line references would have no specific and distinct closest match.  

Next we match parents with daughters. For each parent, we compute a set of variants unique to that parent to serve as a "fingerprint". Then, for each daughter, we compute the proportion of those fingerprint mutations which have any reads supporting that mutation in that daughter. For a correct parent:daughter pair, we typically but not always observe 80% or more of the parental fingerprint mutations found in the daughter. Correct pairings can be clearly determined from visual inspection of the resulting heatmaps.

For each cell line analyzed, below will be a printout of all unique top matches across all clones for each cell line in the study. If the clones match their expected lines, then the printout should simply state that all the top hits for the particular cell line is that very same cell line - ie. for all clones annotated as H1650, the reference lines that matched those clones were all H1650. However if one sample is mismatched - for example one clone is actually HeLa, it would print out cell line X and the mismatched cell line as well (H160 and HeLa). Essentially, this is a quick check for whether the annotations match the data. 
...
```{r mutation_filtering_summary, fig.width=8, fig.height=11, echo = FALSE, cache = T}
variant_data = lapply(1:nrow(tbl), function(x){
		path = tbl$analysis_path[x]
		dat_path = paste0(path, "/proc/variants_final.txt")
		tab = fread(cmd = paste("grep -v", "mutect_filtered", dat_path), dec = ".")[tumor.LAB != "mutect_filtered"]
		tab$name = tbl$name[x]
		return(tab)
})
...
We now show the spectrum of how shared parental mutations are. After removing germline mutations detected by Mutect2, we plot the distribution of the number of parents sharing each of their mutations. Each bar represents the number of mutations (y-axis) shared in that many parents (x-axis). The far left bar (1 parent) represents private parental mutations, whereas the far right bar represents mutations found in all parents. Spikes in the distribution (bars with many mutations in between 1 and the maximum) represent groups of parents that are relatively more related to one another - for example, if a group of knockouts were derived from one parental clone. 
...
variant_data[,mmq2:=tstrsplit(MMQ, split = ",")[2]]
