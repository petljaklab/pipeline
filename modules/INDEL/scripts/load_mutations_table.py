...
geno_col = vcf[it].index(args.sample_id)
...
for line in vcf[(it+1):]:
    ...
    try:
        ...
        continue
    table_to_import.append([genome, line[chrom_col], line[pos_col], line[pos_col], line[ref_col], line[alt_col], ref_depth, alt_depth, args.mutation_type, line[filter_col], "mutect2", str(analysis_id), str(sample_id), str(study_id)])    
