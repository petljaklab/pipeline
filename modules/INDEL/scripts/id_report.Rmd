...
```{r general_overview_plots, fig.height=8, fig.width=11, echo = FALSE, cache = TRUE}
for(c in unique(m2_dat$cell)){
	genotypes = sort(unique(m2_dat[cell == c]$genotype))
	for(geno in genotypes){
			print(ggplot(m2_dat[cell == c][genotype == geno], aes(x = name, fill = tumor.LAB)) + geom_bar() + theme(axis.text.x = element_text(angle = 90, vjust = 0.5)) + scale_fill_discrete(labels = long_label_wrap, name = "Mutation\nAnnotation") + ggtitle(paste0("Filtering of InDel mutations for cell line ", c, "\nCondition: ", geno)) + xlab("Sample") + ylab("Indel Count"))
	}
}
...
```{r fig.height=10, fig.width=8, cache = FALSE, echo = FALSE}
### Plot 20 per page
plt_counter = 0
pltlist = list()
