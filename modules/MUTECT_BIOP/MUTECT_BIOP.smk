...
rule MUTECT2_BIOP:
    input:
        ...
        biop_merge = lambda wildcards: gateway("WGS_MERGE_BAM", wildcards.sample, scratch_dir = SCRATCH_DIR, prod_dir = PROD_DIR, db = "petljakdb")[0],
        germ_merge = lambda wildcards: gateway("WGS_MERGE_BAM", matched_norm(wildcards), scratch_dir = SCRATCH_DIR, prod_dir = PROD_DIR, db = "petljakdb")[0]
    output:
